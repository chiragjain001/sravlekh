'use client';
// ─── AdminStudents — Enterprise Students Module ───────────────────────────────
// Full CRUD + Search + Filter + Sort + Pagination + Bulk Actions
// Analytics Tab | Import | Export | Profile Drawer | Create/Edit/Delete Dialogs
// All data flows through the service layer. Zero hardcoded values.
...
import { useStudentsList, useCreateStudent, useUpdateStudent, useDeleteStudent, useBulkDeleteStudents, useExportStudents } from '@/features/students/hooks/useStudents';
...
import type { StudentListItem, GetStudentsParams, SortDirection, CreateStudentInput, UpdateStudentInput } from '@/features/students/types/student.types';

// ── Constants ──────────────────────────────────────────────────────────────────
const PAGE_SIZES = [10, 20, 50] as const;
const PROGRAMS   = ['JEE', 'NEET', '11th', '12th', 'Foundation'];
const FEE_STATUS = ['paid', 'partial', 'overdue', 'waived'];
...
export function AdminStudents() {
  ...
  const [sortBy,   setSortBy]   = useState<keyof StudentListItem>('name');
  ...
  const params: GetStudentsParams = {
    ...
    feeStatus: feeFilter  ? (feeFilter  as any) : undefined,
    riskLevel: riskFilter ? (riskFilter as any) : undefined,
    sortBy, sortDir,
  };

  // ── Data hooks
  const { data, isLoading, isFetching, refetch } = useStudentsList(params);
  ...
  // ── Sort handler
  function handleSort(col: keyof StudentListItem) {
    if (col === sortBy) {
  ...
  function handleSelectAll() {
    if (students.every((s) => selected.has(s.id))) {
      setSelected(new Set());
    } else {
  ...
  async function handleDeleteConfirm() {
    ...
    showToast(`${ids.length} student(s) removed`);
  }
  ...
  async function handleExport() {
    ...
    showToast(`Exported ${ids?.length ?? totalItems} students`);
  }

  function handleClearFilters() {
    setProgram(''); setFeeFilter(''); setRiskFilter('');
    setSearchInput(''); setPage(1);
  }

  const activeFilters = [program, feeFilter, riskFilter].filter(Boolean).length;
      ...
      <div className="px-6 pt-6 pb-4 bg-white border-b border-slate-100">
        <div className="flex items-center justify-between flex-wrap gap-3 max-w-[1700px] mx-auto">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Manage enrollment, performance, and analytics</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold text-white bg-indigo-600 rounded-lg hover:bg-indigo-700 shadow-sm shadow-indigo-200 transition-colors"
            >
        ...
        {tab === 'analytics' ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <StatCard label="Total Students"  value="1,284" sub="↑7%"   colorClass="text-emerald-600" />
              <StatCard label="Active"          value="1,142" sub="89%"   colorClass="text-blue-600" />
              <StatCard label="At-Risk"         value="86"    sub="↑12%"  colorClass="text-rose-600" />
              <StatCard label="New (This Month)" value="126"  sub="↑15%"  colorClass="text-emerald-600" />
            </div>
            ...
            <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-3">
              ...
              {showFilter && (
                <div className="flex items-center gap-3 flex-wrap mt-3 pt-3 border-t border-slate-100">
                  {[
                    { label: 'Program',     value: program,    set: setProgram,    opts: PROGRAMS.map(o => ({ value: o, label: o })) },
                    { label: 'Fee Status',  value: feeFilter,  set: setFeeFilter,  opts: FEE_STATUS.map(o => ({ value: o, label: o.charAt(0).toUpperCase() + o.slice(1) })) },
                    { label: 'Performance', value: riskFilter, set: setRiskFilter, opts: [

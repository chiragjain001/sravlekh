'use client';
// ─── FounderAuditLogs — global, real backend ───────────────────────────────
// 06-AUTH-AUTHORIZATION.md: FOUNDER gets RW(global) audit access, vs. ADMIN's
// R(tenant). Same redaction as the admin-facing audit screen.

import { useState } from 'react';
import { Shield } from 'lucide-react';
import { useFounderAuditLogs, useFounderInstitutes } from '@/hooks/useApi';
...
export function FounderAuditLogs() {
  ...
  const { data: institutes } = useFounderInstitutes();
  const { data, isPending, isError } = useFounderAuditLogs({
  ...
  const instituteNames = new Map<string, string>((institutes ?? []).map((i: { id: string; name: string }) => [i.id, i.name]));

  return (
    <div className="p-6 space-y-4 max-w-[1300px] mx-auto w-full">
      <div>
        <h2 className="text-[15px] font-bold text-slate-900">Cross-Tenant Audit Log</h2>
        <p className="text-xs text-slate-500 mt-0.5">Every sensitive action across every institute on the platform.</p>
      </div>
      ...
      >
        ...
        {(institutes ?? []).map((i: { id: string; name: string }) => (
          <option key={i.id} value={i.id}>{i.name}</option>
        ))}
      </select>

      {isPending && <SkeletonTable rows={8} cols={5} />}
      {isError && <EmptyState icon={<Shield className="w-6 h-6" />} title="Couldn't load the audit log" />}
      {!isPending && !isError && logs.length === 0 && <EmptyState icon={<Shield className="w-6 h-6" />} title="No matching audit entries" />}
            ...
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                  <td className="px-4 py-3 text-[12px] text-slate-600">{log.instituteId ? instituteNames.get(log.instituteId) ?? log.instituteId : 'Platform-wide'}</td>
                  <td className="px-4 py-3">
                    <span className={`text-[10.5px] font-bold px-2 py-0.5 rounded-md ${ACTION_STYLE[log.action] ?? 'bg-slate-100 text-slate-600'}`}>{log.action.replace(/_/g, ' ')}</span>
                  </td>

...
export function DeleteTeacherDialog({
  isOpen, onClose, onConfirm, teacherNames, loading = false,
}: DeleteTeacherDialogProps) {
  if (!isOpen) return null;

  const isBulk  = teacherNames.length > 1;
  const title   = isBulk ? `Remove ${teacherNames.length} Faculty Members` : `Remove ${teacherNames[0]}`;

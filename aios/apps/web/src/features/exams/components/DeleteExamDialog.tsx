...
export function DeleteExamDialog({
  isOpen, onClose, onConfirm, examNames, loading = false,
}: DeleteExamDialogProps) {
  if (!isOpen) return null;

  const isBulk = examNames.length > 1;
  const title  = isBulk ? `Cancel ${examNames.length} Exams` : `Cancel ${examNames[0]}`;
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-rose-100">
        ...
        <div className="p-5 space-y-4">
          <div className="flex gap-3 p-4 bg-rose-50 rounded-xl border border-rose-100 text-xs text-rose-800 font-medium">
            ...
            <span>
              {isBulk
                ? `Cancelling ${examNames.length} exams will mark their status as Cancelled.`
                : `Cancelling ${examNames[0]} will mark its status as Cancelled.`}
            </span>
          </div>

          <p className="text-[11px] text-slate-400">Registered candidates and draft blueprints remain saved for reference.</p>
        </div>
        ...
        <div className="flex items-center justify-end gap-2 px-5 py-4 border-t border-slate-100 bg-slate-50 rounded-b-2xl">
          ...
          >
            ...
            {loading ? 'Cancelling…' : 'Yes, Cancel Exam'}
          </button>
        </div>
      </div>
    </div>
